...
Cell::Cell(int _value, SDL_Texture *_openTexture, SDL_Texture *_closeTexture) : value(_value),
                                                                                openTexture(_openTexture),
                                                                                closeTexture(_closeTexture) {}

void Cell::draw(int i, int j, SDL_Renderer *renderer) {

    TextureManager::getInstance()->ApplySurface(i * Cell::size + angleX,
                                                j * Cell::size + angleY, closeTexture, renderer);
}
...
int Cell::open(int i, int j, SDL_Renderer *renderer) {
    ...
    if (value != -1 && closeTexture != Cell::flagTexture) {
        ...
        TextureManager::getInstance()->ApplySurface(i * Cell::size + angleX,
                                                    j * Cell::size + angleY, openTexture, renderer);
...
void Cell::show(int i, int j, SDL_Renderer *renderer) {
    ...
    if (closeTexture != Cell::flagTexture)
        ...
        TextureManager::getInstance()->ApplySurface(i * Cell::size + angleX, j * Cell::size + angleY,
                                                    crossedFlagTexture, renderer);
}
